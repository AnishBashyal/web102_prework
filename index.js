...
const totalUnfunded = unfundedGames.reduce((acc, game) => {
  return acc + 1;
}, 0);
// create a string that explains the number of unfunded games using the ternary operator
const displayStr = `A total of $${totalRaised.toLocaleString(
  "en-US"
)} has been raised for ${totalGameCards - totalUnfunded} ${
  totalGameCards > 1 ? "games" : "game"
}. 
Currently, ${totalUnfunded} ${totalGameCards > 1 ? "games" : "game"} ${
  totalGameCards > 1 ? "remain" : "remains"
} unfunded. 
We need your help to fund ${totalGameCards > 1 ? "these" : "this"} amazing ${
  totalGameCards > 1 ? "games" : "game"
}!`;
...
const firstGameEl = document.createElement("h3");
firstGameEl.innerHTML = `${firstGame.name}`;
firstGameContainer.appendChild(firstGameEl);
// do the same for the runner up item
const secondGameEl = document.createElement("h3");
